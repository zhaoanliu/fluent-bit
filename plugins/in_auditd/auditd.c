...
/* cb_collect callback */
static int in_auditd_collect_pending(struct flb_input_instance *i_ins,
                                   struct flb_config *config, void *in_context)
{
    ...
    mk_list_foreach_safe(head, tmp, &ctx->files_event) {
        ...
        ret = flb_auditd_file_chunk(file);
        switch (ret) {
        case FLB_AUDITD_ERROR:
            /* Could not longer read the file */
            flb_auditd_file_remove(file);
            break;
        case FLB_AUDITD_OK:
        case FLB_AUDITD_BUSY:
            /*
             * Adjust counter to verify if we need a further read(2) later.
             * For more deauditds refer to auditd_fs_inotify.c:96.
             */
...
/* cb_collect callback */
static int in_auditd_collect_static(struct flb_input_instance *i_ins,
                                  struct flb_config *config, void *in_context)
{
    int ret;
    int active = 0;
    ...
    mk_list_foreach_safe(head, tmp, &ctx->files_static) {
        ...
        case FLB_AUDITD_BUSY:
            active++;
            continue;
        case FLB_AUDITD_WAIT:
            if (file->config->exit_on_eof) {
                flb_info("[in_auditd] file=%s ended, stop", file->name);
                flb_engine_exit(config);
            }
            /* Promote file to 'events' type handler */
            flb_debug("[in_auditd] file=%s promote to AUDITD_EVENT", file->name);
            ret = flb_auditd_file_to_event(file);
            if (ret == -1) {
                flb_debug("[in_auditd] file=%s cannot promote, unregistering",
                          file->name);
                flb_auditd_file_remove(file);
            }
            break;
...
{
    int ret;
    struct flb_auditd_file *f = file;
    flb_debug("START in_auditd_collect_event");
    flb_debug("[in_auditd] file=%s event", f->name);

    ret = flb_auditd_file_chunk(f);
    switch (ret) {
    case FLB_AUDITD_ERROR:
        /* Could not longer read the file */
        flb_auditd_file_remove(f);
        break;
    case FLB_AUDITD_OK:
    case FLB_AUDITD_WAIT:
        break;
...
/* Initialize plugin */
static int in_auditd_init(struct flb_input_instance *in,
                        struct flb_config *config, void *data)
{
    int ret = -1;
    struct flb_auditd_config *ctx = NULL;

    /* Allocate space for the configuration */
    ctx = flb_auditd_config_create(in, config);
    if (!ctx) {
        return -1;
    }
    ctx->i_ins = in;
    ...
    ret = flb_auditd_fs_init(in, ctx, config);
    if (ret == -1) {
        flb_auditd_config_destroy(ctx);
        return -1;
    }

    /* Scan path */
    flb_info("[in_auditd] in_auditd_init, before flb_auditd_scan");
    flb_auditd_scan(ctx->path, ctx);
    flb_info("[in_auditd] in_auditd_init, after flb_auditd_scan");
    flb_trace("[in_auditd] path: %s", ctx->path);

    /* Set plugin context */
    flb_input_set_context(in, ctx);

    /* Register an event collector */
    ret = flb_input_set_collector_event(in, in_auditd_collect_static,
    ...
    /* Register re-scan */
    ret = flb_input_set_collector_time(in, flb_auditd_scan_callback,
    ...
    ret = flb_input_set_collector_time(in, flb_auditd_file_rotated_purge,
                                       ctx->rotate_wait, 0,
                                       config);
    if (ret == -1) {
        flb_auditd_config_destroy(ctx);
    ...
    ret = flb_input_set_collector_event(in, in_auditd_collect_pending,
    ...
    if (ctx->multiline == FLB_TRUE && ctx->parser) {
        ctx->parser = NULL;
        flb_warn("[in_auditd] on multiline mode 'Parser' is not allowed "
                 "(parser disabled)");
    }

    /* Register callback to process docker mode queued buffer */
    if (ctx->docker_mode == FLB_TRUE) {
        ret = flb_input_set_collector_time(in, flb_auditd_dmode_pending_flush,
    ...
    if (ctx->multiline == FLB_TRUE) {
        ret = flb_input_set_collector_time(in, flb_auditd_mult_pending_flush,
...
#endif
    flb_info("[in_auditd] END in_auditd_init");
    return 0;
}

/* Pre-run callback / before the event loop */
static int in_auditd_pre_run(struct flb_input_instance *i_ins,
                           struct flb_config *config, void *in_context)
{
    struct flb_auditd_config *ctx = in_context;
    (void) i_ins;

    return auditd_signal_manager(ctx);
}

static int in_auditd_exit(void *data, struct flb_config *config)
{
    (void) *config;
    struct flb_auditd_config *ctx = data;

    if (ctx->exclude_list) {
        flb_utils_split_free(ctx->exclude_list);
    }

    flb_auditd_file_remove_all(ctx);
    flb_auditd_config_destroy(ctx);

    return 0;
}

static void in_auditd_pause(void *data, struct flb_config *config)
{
    struct flb_auditd_config *ctx = data;
    ...
    flb_auditd_fs_pause(ctx);
}

static void in_auditd_resume(void *data, struct flb_config *config)
{
    struct flb_auditd_config *ctx = data;
    ...
    /* Pause file system backend handlers */
    flb_auditd_fs_resume(ctx);
}

struct flb_input_plugin in_auditd_plugin = {
    .name         = "auditd",
    .description  = "auditd input plugin",
    .cb_init      = in_auditd_init,
    .cb_pre_run   = in_auditd_pre_run,
    .cb_collect   = NULL,
    .cb_flush_buf = NULL,
    .cb_pause     = in_auditd_pause,
    .cb_resume    = in_auditd_resume,
    .cb_exit      = in_auditd_exit,
    .flags        = 0
};
