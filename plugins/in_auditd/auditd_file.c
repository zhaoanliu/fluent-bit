...
#include "auditd_dockermode.h"
#include "auditd_multiline.h"
#include "auditd_scan.h"
...
static int get_inode(int fd, uint64_t *inode)
{
    ...
    if (h == INVALID_HANDLE_VALUE) {
        flb_error("[in_auditd] cannot convert fd:%i into HANDLE", fd);
        return -1;
    }

    if (GetFileInformationByHandle(h, &info) == 0) {
        flb_error("[in_auditd] cannot get file info for fd:%i", fd);
        return -1;
    }
...
int flb_auditd_pack_line_map(msgpack_sbuffer *mp_sbuf, msgpack_packer *mp_pck,
                           struct flb_time *time, char **data,
                           size_t *data_size, struct flb_auditd_file *file)
{
    int map_num = 1;
...
int flb_auditd_file_pack_line(msgpack_sbuffer *mp_sbuf, msgpack_packer *mp_pck,
                            struct flb_time *time, char *data, size_t data_size,
                            struct flb_auditd_file *file)
{
    int map_num = 1;
    struct flb_auditd_config *ctx = file->config;
...
static void cb_id(const char *name, const char *value,
                       size_t vlen, void *data)
{
    if (strcmp(name, "id") == 0 && vlen > 0)
    {
        strncpy(data, value, vlen);
    }
}

static int process_content(struct flb_auditd_file *file, off_t *bytes)
{
    int len;
    int lines = 0;
    int ret;
...
//////////////////////////////////////////////////////////////////////////////////////////
        char currentLine[line_len + 1];
        strncpy(currentLine, line, line_len);
        currentLine[line_len] = '\0';
        flb_info("[in_auditd] process_content: len=%i currentLine:%s", line_len, currentLine);
        
        char newAuditId[30] = {0};

#ifdef FLB_HAVE_REGEX
        struct flb_regex_search result;
        ssize_t n;

        n = flb_regex_do(ctx->auditid_regex, line, line_len, &result);
        if (n <= 0) {
            flb_debug("[in_auditd] regex not match.");
        }
        else
        {
            flb_debug("[in_auditd] regex match.");
            flb_regex_parse(ctx->auditid_regex, &result, cb_id, newAuditId);
            flb_debug("[in_auditd] id: %s", newAuditId[0] == '\0' ? "NULL" : newAuditId);
        }
#endif
//////////////////////////////////////////////////////////////////////////////////////////

        if (ctx->docker_mode) {
            ret = flb_auditd_dmode_process_content(now, line, line_len,
            ...
            if (ret >= 0) {
                ...
                flb_auditd_dmode_flush(out_sbuf, out_pck, file, ctx);
            }
        }
        ...
        if (ctx->parser) {
            ...
            if (ret >= 0) {
                ...
                if (ctx->multiline == FLB_TRUE) {
                    flb_auditd_mult_flush(out_sbuf, out_pck, file, ctx);
                }

                flb_auditd_pack_line_map(out_sbuf, out_pck, &out_time,
                ...
                flb_time_get(&out_time);
                flb_auditd_file_pack_line(out_sbuf, out_pck, &out_time,
                                        data, len, file);
            }
        }
        else if (ctx->multiline == FLB_TRUE) {
            ret = flb_auditd_mult_process_content(now,
                                                line, line_len, 
                                                currentAuditId, newAuditId,
                                                file, ctx);

            /* No multiline */
            if (ret == FLB_AUDITD_MULT_NA) {
            ...
            else if (ret == FLB_AUDITD_MULT_MORE) {
                /* we need more data, do nothing */
                goto go_next;
            }
            else if (ret == FLB_AUDITD_MULT_DONE) {
                /* Finalized */
            }
        }
        else {
...
#else
        flb_debug("[in_auditd] NOT FLB_HAVE_PARSER.");
        flb_time_get(&out_time);
        flb_auditd_file_pack_line(out_sbuf, out_pck, &out_time,
                                line, line_len, file);
#endif
...
#endif
{
    ...
    if (tag_regex) {
        ...
        if (n <= 0) {
            flb_error("[in_auditd] invalid pattern for given file %s", fname);
            return -1;
        }
        else {
            ...
            for (p = tag, beg = p; (beg = strchr(p, '<')); p = end + 2) {
                ...
                if (end && !memchr(beg, '<', end - beg)) {
                    ...
                    flb_error("[in_auditd] missing closing angle bracket in tag %s at position %i", tag, beg - tag);
                    flb_hash_destroy(ht);
                    return -1;
                }
            }
...
int flb_auditd_file_exists(char *name, struct flb_auditd_config *ctx)
{
    struct mk_list *head;
    struct flb_auditd_file *file;
...
int flb_auditd_file_append(char *path, struct stat *st, int mode,
                         struct flb_auditd_config *ctx)
{
    ...
    mk_list_foreach(head, &ctx->files_static) {
        file = mk_list_entry(head, struct flb_auditd_file, _head);
        if (flb_auditd_file_name_cmp(path, file) == 0) {
            return -1;
        }
    }
    mk_list_foreach(head, &ctx->files_event) {
    ...
    if (fd == -1) {
        flb_errno();
        flb_error("[in_auditd] could not open %s", path);
        return -1;
    }

    file = flb_calloc(1, sizeof(struct flb_auditd_file));
    if (!file) {
        flb_errno();
        goto error;
    }
     ...
     */
    ret = flb_auditd_file_name_dup(path, file);
    if (!file->name) {
        flb_errno();
        goto error;
    }
    ...
    char *name = flb_auditd_file_name(file);
    if (name == NULL) {
        goto error;
    }
    if (flb_auditd_file_exists(name, ctx)) {
        flb_free(name);
        goto error;
    }
    flb_free(name);
    ...
    if (ctx->dynamic_tag == FLB_TRUE) {
        ...
        if (!tag) {
            flb_errno();
            flb_error("[in_auditd] failed to allocate tag buffer");
            goto error;
        }
#ifdef FLB_HAVE_REGEX
        ...
        flb_free(tag);
        if (ret != 0) {
            flb_error("[in_auditd] failed to compose tag for file: %s", path);
            goto error;
        }
    }
    else {
    ...
    if (!file->tag_buf) {
        flb_error("[in_auditd] failed to set tag for file: %s", path);
        flb_errno();
        goto error;
    }

    /* Register this file into the fs_event monitoring */
    ret = flb_auditd_fs_add(file);
    if (ret == -1) {
        flb_error("[in_auditd] could not register file into fs_events");
        goto error;
    }

    if (mode == FLB_AUDITD_STATIC) {
        mk_list_add(&file->_head, &ctx->files_static);
    }
    else if (mode == FLB_AUDITD_EVENT) {
...
#ifdef FLB_HAVE_SQLDB
    if (ctx->db) {
        flb_auditd_db_file_set(file, ctx);
    }
#endif
    ...
    flb_metrics_sum(FLB_AUDITD_METRIC_F_OPENED, 1, ctx->i_ins->metrics);
#endif

    flb_debug("[in_auditd] add to scan queue %s, offset=%lu", path, file->offset);
    return 0;
...
void flb_auditd_file_remove(struct flb_auditd_file *file)
{
    if (file->rotated > 0) {
...
#ifdef FLB_HAVE_METRICS
    flb_metrics_sum(FLB_AUDITD_METRIC_F_CLOSED, 1,
...
int flb_auditd_file_remove_all(struct flb_auditd_config *ctx)
{
    int count = 0;
    struct mk_list *head;
    struct mk_list *tmp;
...
int flb_auditd_file_chunk(struct flb_auditd_file *file)
{
    int ret;
    char *tmp;
    size_t size;
    ...
    if (capacity < 1) {
        ...
        if (file->buf_size >= ctx->buf_max_size) {
            if (ctx->skip_long_lines == FLB_FALSE) {
                flb_error("[in_auditd] file=%s requires a larger buffer size, "
                          "lines are too long. Skipping file.", file->name);
                return FLB_AUDITD_ERROR;
            }

            /* Warn the user */
            if (file->skip_warn == FLB_FALSE) {
                flb_warn("[in_auditd] file=%s have long lines. "
            ...
            if (tmp) {
                flb_trace("[in_auditd] file=%s increase buffer size %lu => %lu bytes",
                ...
                flb_errno();
                flb_error("[in_auditd] cannot increase buffer size for %s, "
    ...
    if (bytes > 0) {
        ...
        if (ret >= 0) {
            flb_debug("[in_auditd] file=%s read=%lu lines=%i",
                      file->name, bytes, ret);
        }
        else {
            flb_debug("[in_auditd] file=%s ERROR", file->name);
            return FLB_AUDITD_ERROR;
        }
...
#ifdef FLB_HAVE_SQLDB
        if (file->config->db) {
            flb_auditd_db_file_offset(file, file->config);
        }
#endif

        /* Data was consumed but likely some bytes still remain */
        return FLB_AUDITD_OK;
    }
    else if (bytes == 0) {
        /* We reached the end of file, let's wait for some incoming data */
        return FLB_AUDITD_WAIT;
    }
    else {
        /* error */
        flb_errno();
        flb_error("[in_auditd] error reading %s", file->name);
        return FLB_AUDITD_ERROR;
    }
...
int flb_auditd_file_to_event(struct flb_auditd_file *file)
{
    int ret;
    char *name;
    struct stat st;
    ...
    if (file->offset < st.st_size) {
        ...
        auditd_signal_pending(file->config);
    }
    else {
        file->pending_bytes = 0;
    }
    ...
    if (!name) {
        flb_debug("[in_auditd] cannot detect if file was rotated: %s",
                  file->name);
        return -1;
    }
    ...
    if (flb_auditd_file_name_cmp(name, file) != 0) {
        ...
        if (ret == -1) {
            ...
            flb_trace("[in_auditd] static file rotated: %s => to %s",
                      file->name, name);
    ...
    file->auditd_mode = FLB_AUDITD_EVENT;
 ...
 */
char *flb_auditd_file_name(struct flb_auditd_file *file)
{
    int ret;
    char *buf;
#ifdef __linux__
...
int flb_auditd_file_name_dup(char *path, struct flb_auditd_file *file)
{
    ...
    file->real_name = flb_auditd_file_name(file);
    if (!file->real_name) {
        flb_errno();
        flb_free(file->name);
        file->name = NULL;
...
int flb_auditd_file_rotated(struct flb_auditd_file *file)
{
    int ret;
...
#ifdef FLB_HAVE_METRICS
    flb_metrics_sum(FLB_AUDITD_METRIC_F_ROTATED,
    ...
    flb_debug("[in_auditd] rotated: %s -> %s",
              file->name, name);

    /* Rotate the file in the database */
#ifdef FLB_HAVE_SQLDB
    if (file->config->db) {
        ret = flb_auditd_db_file_rotate(name, file, file->config);
        if (ret == -1) {
            flb_error("[in_auditd] could not rotate file %s->%s in database",
                      file->name, name);
        }
    }
#endif
    ...
    tmp        = file->name;
    flb_auditd_file_name_dup(name, file);
    ...
    if (create == FLB_TRUE) {
        flb_auditd_scan(ctx->path, ctx);
        auditd_signal_manager(file->config);
    }
    flb_free(tmp);
    flb_free(name);
...
int flb_auditd_file_rotated_purge(struct flb_input_instance *i_ins,
                                struct flb_config *config, void *context)
{
    ...
    mk_list_foreach_safe(head, tmp, &ctx->files_rotated) {
        file = mk_list_entry(head, struct flb_auditd_file, _rotate_head);
        if ((file->rotated + ctx->rotate_wait) <= now) {
            flb_debug("[in_auditd] purge rotated file %s", file->name);
            if (file->pending_bytes > 0 && flb_input_buf_paused(i_ins)) {
                flb_warn("[in_auditd] purged rotated file while data ingestion is paused, consider increasing rotate_wait");
            }
            flb_auditd_file_remove(file);
            count++;
        }
