                         ...
                         struct flb_config *config, void *in_context)
{
    ...
    if (ev.mask & IN_ATTRIB) {
        ...
        if (st.st_nlink == 0) {
            flb_debug("[in_auditd] removed %s", file->name);
            flb_auditd_file_remove(file);
            return 0;
        }
    }
    ...
    if (ev.mask & IN_MODIFY) {
        ...
        /* Collect the data */
        ret = in_auditd_collect_event(file, config);
        if (ret != FLB_AUDITD_ERROR) {
            /*
                     ...
                     struct flb_auditd_config *ctx, struct flb_config *config)
{
    ...
    flb_debug("[in_auditd] inotify watch fd=%i", fd);
    ctx->fd_notify = fd;

    /* This backend use Fluent Bit event-loop to trigger notifications */
    ret = flb_input_set_collector_event(in, auditd_fs_event,
...
int flb_auditd_fs_add(struct flb_auditd_file *file)
{
    ...
    if (watch_fd == -1) {
        flb_errno();
        if (errno == ENOSPC) {
            flb_error("[in_auditd] inotify: The user limit on the total number "
