                                               ...
                                               struct flb_config *config)
{
    ...
    ctx = flb_calloc(1, sizeof(struct flb_auditd_config));
    if (!ctx) {
        flb_errno();
        return NULL;
    }
    ...
    if (!ctx->path) {
        flb_error("[in_auditd] no input 'path' was given");
        flb_free(ctx);
        return NULL;
    }
    ...
    if (!tmp) {
        ctx->refresh_interval_sec = FLB_AUDITD_REFRESH;
        ...
        if (ret == 0) {
            ...
            if (sec == 0 && nsec == 0) {
                flb_error("[in_auditd] invalid 'refresh_interval' config value (%s)",
                          tmp);
                flb_free(ctx);
                return NULL;
            }
            ...
            if (sec == 0 && nsec <= 1000000) {
                flb_warn("[in_auditd] very low refresh_interval (%i.%lu nanoseconds) "
    ...
    if (!tmp) {
        ctx->rotate_wait = FLB_AUDITD_ROTATE_WAIT;
    }
    else {
        ctx->rotate_wait = atoi(tmp);
        if (ctx->rotate_wait <= 0) {
            flb_error("[in_auditd] invalid 'rotate_wait' config value");
            flb_free(ctx);
            return NULL;
        }
    }
    ...
    ret = flb_auditd_mult_create(ctx, i_ins, config);
    if (ret == -1) {
        flb_auditd_config_destroy(ctx);
        return NULL;
    }
    ...
    if (tmp) {
        ...
        if (ret == FLB_TRUE) {
            ...
            ret = flb_auditd_dmode_create(ctx, i_ins, config);
            if (ret == -1) {
                flb_auditd_config_destroy(ctx);
                return NULL;
            }
    ...
    if (tmp) {
        ...
        if (bytes > 0) {
            ...
            ctx->buf_chunk_size = FLB_AUDITD_CHUNK;
        }
    }
    else {
        ctx->buf_chunk_size = FLB_AUDITD_CHUNK;
    ...
    if (tmp) {
        ...
        if (bytes > 0) {
            ...
            ctx->buf_max_size = FLB_AUDITD_CHUNK;
        }
    }
    else {
        ctx->buf_max_size = FLB_AUDITD_CHUNK;
    ...
    if (ctx->buf_chunk_size > ctx->buf_max_size) {
        flb_error("[in_auditd] buffer_max_size must be >= buffer_chunk");
        flb_free(ctx);
        return NULL;
    }
    ...
    if (tmp) {
        ...
        if (!ctx->parser) {
            flb_error("[in_auditd] parser '%s' is not registered", tmp);
        }
    }
#endif
    ...
    if (tmp) {
        ...
        if (ctx->tag_regex) {
            ...
            flb_error("[in_auditd] invalid 'tag_regex' config value");
        }
    }
    else {
        ctx->tag_regex = NULL;
    ...
    /* Create regex to parse audit id from audit log lines */
    const char *p_regex = "^type=\\S+\\smsg=audit\\((?<id>\\d+.\\d+:\\d+)\\):.*";
    ctx->auditid_regex = flb_regex_create(p_regex);
#endif
    ...
    if (tmp) {
        if (strcasecmp(tmp, "extra") == 0) {
            ...
            flb_error("[in_auditd] invalid database 'db.sync' value");
        }
    }
    ...
    if (tmp) {
        ctx->db = flb_auditd_db_open(tmp, i_ins, ctx, config);
        if (!ctx->db) {
            flb_error("[in_auditd] could not open/create database");
        }
    }
#endif

#ifdef FLB_HAVE_METRICS
    flb_metrics_add(FLB_AUDITD_METRIC_F_OPENED,
                    "files_opened", ctx->i_ins->metrics);
    flb_metrics_add(FLB_AUDITD_METRIC_F_CLOSED,
                    "files_closed", ctx->i_ins->metrics);
    flb_metrics_add(FLB_AUDITD_METRIC_F_ROTATED,
...
int flb_auditd_config_destroy(struct flb_auditd_config *config)
{
...
#ifdef FLB_HAVE_PARSER
    flb_auditd_mult_destroy(config);
#endif
    ...
    if (config->auditid_regex) {
        flb_regex_destroy(config->auditid_regex);
    }
#endif

#ifdef FLB_HAVE_SQLDB
    if (config->db != NULL) {
        flb_auditd_db_close(config->db);
    }
#endif
