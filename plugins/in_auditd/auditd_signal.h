...
#ifndef FLB_AUDITD_SIGNAL_H
#define FLB_AUDITD_SIGNAL_H

#include "auditd_config.h"

static inline int auditd_signal_manager(struct flb_auditd_config *ctx)
{
    int n;
    uint64_t val = 0xc001;
...
static inline int auditd_signal_pending(struct flb_auditd_config *ctx)
{
    int n;
    uint64_t val = 0xc002;
...
static inline int auditd_consume_pending(struct flb_auditd_config *ctx)
{
    int ret;
    uint64_t val;
