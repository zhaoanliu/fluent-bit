                          ...
                          struct flb_config *config)
{
    ...
    if (ctx->multiline == FLB_TRUE) {
        flb_error("[in_auditd] Docker mode cannot be enabled when multiline is enabled");
        return -1;
    }

    tmp = flb_input_get_property("docker_mode_flush", i_ins);
    if (!tmp) {
        ctx->docker_mode_flush = FLB_AUDITD_DMODE_FLUSH;
    }
    else {
                          ...
                          struct flb_auditd_file *file, struct flb_auditd_config *ctx)
{
    ...
    if (ctx->parser) {
        ...
        if (ret >= 0) {
            ...
            flb_auditd_pack_line_map(mp_sbuf, mp_pck, &out_time,
                                   (char**) &out_buf, &out_size, file);
            goto dmode_flush_end;        }
    }
#endif
    flb_time_get(&out_time);
    flb_auditd_file_pack_line(mp_sbuf, mp_pck, &out_time,
                                 ...
                                 struct flb_config *config, void *context)
{
    ...
    mk_list_foreach(head, &ctx->files_event) {
        ...
        flb_auditd_dmode_flush(&mp_sbuf, &mp_pck, file, ctx);
