...
/* Open or create database required by auditd plugin */
struct flb_sqldb *flb_auditd_db_open(const char *path,
                                   ...
                                   struct flb_config *config)
{
    ...
    if (ret != FLB_OK) {
        flb_error("[in_auditd:db] could not create 'track' table");
        flb_sqldb_close(db);
        return NULL;
    }
    ...
    if (ctx->db_sync >= 0) {
        ...
        if (ret != FLB_OK) {
            flb_error("[in_auditd:db] could not set pragma 'sync'");
            flb_sqldb_close(db);
            return NULL;
        }
    }
    ...
    if (ret != FLB_OK) {
        flb_error("[in_auditd:db] could not set pragma 'journal_mode'");
        flb_sqldb_close(db);
        return NULL;
    }
...
int flb_auditd_db_close(struct flb_sqldb *db)
{
    flb_sqldb_close(db);
    return 0;
}
...
int flb_auditd_db_file_set(struct flb_auditd_file *file,
                         struct flb_auditd_config *ctx)
{
    int ret;
    char query[PATH_MAX];
...
/* Update offset */
int flb_auditd_db_file_offset(struct flb_auditd_file *file,
                            struct flb_auditd_config *ctx)
{
    int ret;
    char query[PATH_MAX];
...
int flb_auditd_db_file_rotate(const char *new_name,
